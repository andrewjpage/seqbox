...
select sample.sample_identifier, sample.day_received, sample.month_received, sample.year_received,
       pr.pcr_result as qech_pcr_result, pr.ct as original_ct, project_name, e.extraction_identifier,
       DATE(e.date_extracted) as date_extracted, ccp.pcr_identifier, DATE(ccp.date_pcred) as date_covid_confirmatory_pcred,
       ccp.ct as covid_confirmation_pcr_ct, tp.pcr_identifier as tiling_pcr_identifier,
       DATE(tp.date_pcred) as date_tiling_pcrer, rsb.name as read_set_batch_name, r.readset_identifier, acr.pct_covered_bases
from sample
...
select project_name, sample_source_identifier, sample_identifier, ct as confirmatory_pcr_ct, readset_identifier, barcode, name as batch_name, pct_covered_bases, lineage, scorpio_call, tp.protocol from read_set
left join read_set_nanopore rsn on read_set.id = rsn.readset_id
left join read_set_batch rsb on read_set.readset_batch_id = rsb.id
left join raw_sequencing rs on read_set.raw_sequencing_id = rs.id
left join tiling_pcr tp on rs.tiling_pcr_id = tp.id
...
left join sample_source_project ssp on ss.id = ssp.sample_source_id
left join project p on ssp.project_id = p.id
where name = '20220217_1544_MN34547_FAQ93072_25640687';
