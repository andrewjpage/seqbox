select sample.sample_identifier, e.extraction_identifier, DATE(e.date_extracted), ccp.pcr_identifier, DATE(ccp.date_pcred),
       ccp.ct, tp.pcr_identifier, DATE(tp.date_pcred), rsb.name, r.seqbox_id, r.read_set_filename
from sample
...
order by e.date_extracted desc, ccp.date_pcred desc, tp.date_pcred desc, rsb.name desc, r.seqbox_id desc;